...
describe("Go and exact-pinned Gateway differential", () => {
  ...
  it("matches provider definitions, MCP continuation and hosted unary output", async () => {
    const serverBody = { content: [
      { type: "tool-call", toolCallId: "call", toolName: "echo", input: "{}", providerExecuted: true, dynamic: true, providerMetadata: { anthropic: { type: "mcp-tool-use", serverName: "echo" } } },
      { type: "tool-result", toolCallId: "call", toolName: "echo", result: { value: 0 }, isError: true, providerMetadata: { anthropic: { type: "mcp-tool-use", serverName: "echo" } } },
    ...
    try {
      const options: LanguageModelV4CallOptions = {
        prompt: [{ role: "assistant", content: [
          { type: "tool-call", toolCallId: "history", toolName: "echo", input: {}, providerExecuted: true, providerOptions: { anthropic: { type: "mcp-tool-use", serverName: "echo" } } },
          { type: "tool-result", toolCallId: "history", toolName: "echo", output: { type: "json", value: { ok: true } }, providerOptions: { anthropic: { type: "mcp-tool-use", serverName: "echo" } } },
        ] }],
        tools: [{ type: "provider", id: "anthropic.code_execution_20260120", name: "code", args: {} }],
        providerOptions: { anthropic: { mcpServers: [{ type: "url", name: "echo", url: "https://mcp.example.test", authorizationToken: "dummy" }] } },
      };
  ...
  it("matches hosted stream markers, preliminary results and result-only continuation", async () => {
    const events = [
      ...
      { type: "tool-call", toolCallId: "call", toolName: "echo", input: "{}", providerExecuted: true, dynamic: true, providerMetadata: { anthropic: { type: "mcp-tool-use", serverName: "echo" } } },
      { type: "tool-result", toolCallId: "call", toolName: "echo", result: "preview", preliminary: true, providerMetadata: { anthropic: { type: "mcp-tool-use", serverName: "echo" } } },
