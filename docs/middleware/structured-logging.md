...
preserves the greatest value observed for each normalized counter. Prompt text,
output text, reasoning, tool payloads, files, headers, bodies, provider options,
and raw chunks are not captured by default.
