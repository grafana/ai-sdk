...
func TestStreamFrameEncoding(t *testing.T) {
	t.Run("canonical fallbacks", func(t *testing.T) {
		...
		internal, ok := encodeStreamFrame(streamEvent{typeName: provider.PartError, safeError: safeError{category: safeInternal}}, 1<<20)
		require.True(t, ok)
		assert.Equal(t, canonicalInternalStreamErrorFrame, internal)
	})
	...
	t.Run("normalized warnings preserve required values", func(t *testing.T) {
		warnings, err := mapWarnings([]provider.Warning{
		...
		assert.Contains(t, string(frame), warningUnsupportedDetails)
		assert.Contains(t, string(frame), warningCompatibilityDetails)
		assert.Contains(t, string(frame), warningDeprecatedMessage)
		assert.Contains(t, string(frame), warningOtherMessage)
	})
}
...
func TestStreamingRuntimeStartNormalization(t *testing.T) {
	t.Run("provider start is consumed exactly once", func(t *testing.T) {
		...
		assert.Contains(t, response.Body.String(), warningDeprecatedMessage)
	})
	...
	t.Run("api error status and wrapped transport are classified safely", func(t *testing.T) {
		tests := []struct {
		...
		}{
			{
				...
				apiError:  provider.NewAPICallError(provider.APICallErrorOptions{StatusCode: 700, Message: "private", Cause: testNetError{timeout: true}}),
			...
			},
			{
				...
				apiError:  provider.NewAPICallError(provider.APICallErrorOptions{StatusCode: http.StatusOK, Message: "private", Cause: testNetError{timeout: true}}),
				wantError: `"error":{"message":"upstream failure","type":"internal_server_error","param":null,"code":"upstream_error","statusCode":502,"retryable":true}`,
			},
		}
		for _, tc := range tests {
