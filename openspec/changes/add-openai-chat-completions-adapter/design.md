...
- Host builds immutable canonical-ID policies from provider type and exact backend model ID before middleware hides backend identity. Unknown IDs fail closed on native requests.
- Native Bearer delegates to existing access-token verification; X-Access-Token, X-Grafana-Id and X-Scope-* are rejected in access-token mode. Cloud mode reuses existing credential-rejection and trusted assertion verification unchanged.
...
Additive route. Existing configuration and ProviderWire clients stay valid. Native route reuses the configured limits; rollout documentation lists exact unsupported fields and model profiles. Rollback removes native composition without modifying the shared foundations.
