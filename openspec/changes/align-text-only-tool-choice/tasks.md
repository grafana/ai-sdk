...
- [x] 3.1 Add exact `ai@7.0.65` to `ai-gateway/test/providerwire-v4/package.json` and update the existing `test/pnpm-lock.yaml` with normal workspace tooling; preserve registered package pins and release-age checks. Add actual TypeScript `streamText` calls, not reconstructed `doStream` options.
