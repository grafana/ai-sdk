...
### Requirement: Makefile targets for local development
The system SHALL add Makefile targets for running the integration tests locally.

#### Scenario: Run integration tests via make
- **WHEN** a developer runs `make test-integration`
- **THEN** the Go test server is built, TS dependencies are installed (if needed), and vitest runs the integration suite

#### Scenario: Run CLI via make
- **WHEN** a developer runs `make test-cli URL=http://localhost:8080/scenario/simple-text`
