...
The `provider` package SHALL define a `ToolResultContentType` typed string with constants: `ToolContentText` ("text"), `ToolContentFileData` ("file-data"), `ToolContentFileURL` ("file-url"), `ToolContentFileID` ("file-id"), `ToolContentImageData` ("image-data"), `ToolContentImageURL` ("image-url"), `ToolContentImageFileID` ("image-file-id"), `ToolContentCustom` ("custom"). The `ToolResultContentValue.Type` field SHALL be typed as `ToolResultContentType`. The `ToolResultContentValue.FileID` field (json `"fileId"`) SHALL hold the provider-specific file identifier, matching the upstream `fileId` field.
...
The `provider` package SHALL define a `ReasoningEffort` typed string with constants: `ReasoningProviderDefault` ("provider-default"), `ReasoningNone` ("none"), `ReasoningMinimal` ("minimal"), `ReasoningLow` ("low"), `ReasoningMedium` ("medium"), `ReasoningHigh` ("high"), `ReasoningXHigh` ("xhigh"). The `CallOptions.Reasoning` field SHALL be typed as `*ReasoningEffort`. The existing constant names SHALL be preserved.

#### Scenario: Reasoning field uses typed pointer
- **WHEN** a caller sets `CallOptions.Reasoning`
- **THEN** it SHALL assign a `*ReasoningEffort` value, e.g., `r := ReasoningHigh; opts.Reasoning = &r`

#### Scenario: Anthropic reasoning maps use typed keys
- **WHEN** the Anthropic provider's `reasoningEffortToThinking` and `reasoningEffortToFraction` maps are defined
- **THEN** their key type SHALL be `ReasoningEffort`, not bare `string`

#### Scenario: ReasoningEffort JSON round-trip
- **WHEN** `CallOptions{Reasoning: &ReasoningHigh}` is marshaled to JSON
- **THEN** it SHALL produce `"reasoning":"high"`, identical to the previous bare string behavior
