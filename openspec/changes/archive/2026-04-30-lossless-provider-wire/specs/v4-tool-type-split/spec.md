...
- **WHEN** a function-typed `Tool` is constructed with `Type: ToolTypeFunction`, `Name: "get_weather"`, `Description: "Gets weather"`, an `InputSchema`, one `InputExamples` entry, and `Strict: true`
