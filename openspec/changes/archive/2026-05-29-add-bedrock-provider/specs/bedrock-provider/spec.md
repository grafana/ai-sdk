...
When the consumer requests JSON response format with a schema and the model supports native structured output, the provider SHALL configure native JSON-schema output via `additionalModelRequestFields.output_config.format` instead of injecting a synthetic JSON tool.
