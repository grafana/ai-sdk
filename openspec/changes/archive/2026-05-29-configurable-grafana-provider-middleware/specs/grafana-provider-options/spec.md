...
At minimum it SHALL include a `Sigil` field of type `*SigilControl`. It MAY
include additional per-middleware control fields (for example `Tracing`,
`Metrics`, `Usage`) as those middlewares are introduced.
...
side-effects (for sigil, no generation record is created). The `Disabled` knob is
...
### Requirement: Graded sigil capture control

The `providers/grafana` package SHALL define a `SigilControl` struct carrying
...
sigil capture-mode set, including at least full capture, metadata-only capture,
...
sigil capture-mode set has no "off" value; full suppression of sigil for a
request SHALL be expressed via `Disabled`.

#### Scenario: Capture mode is a typed enum

- **WHEN** the `SigilControl.CaptureMode` field type is inspected
...
- **WHEN** a client constructs a `SigilControl`
...
- **WHEN** a client sets `SigilControl.CaptureMode` to the metadata-only value for a sensitive conversation
- **THEN** the option SHALL express that the request be recorded by sigil in metadata-only mode while other requests retain their backend default

#### Scenario: Fully disabling sigil uses Disabled, not CaptureMode

- **WHEN** a client wants no sigil generation record at all for a sensitive request
- **THEN** it SHALL set `SigilControl.Disabled` to true rather than relying on any `CaptureMode` value, since no capture mode suppresses the record
...
- **WHEN** a client constructs a `SigilControl` with a `CaptureMode` value that is not one of the defined constants and validation runs
...
middleware for the request; and a graded field (e.g. sigil `CaptureMode`)
...
- **WHEN** a `GrafanaOptions.Sigil.CaptureMode` is set to a defined value
