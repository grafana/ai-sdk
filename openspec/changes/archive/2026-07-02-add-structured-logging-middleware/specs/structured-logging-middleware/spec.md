...
The module SHALL depend on the root `github.com/grafana/ai-sdk` module and the Go standard library. It SHALL NOT introduce dependencies on OpenTelemetry SDKs, Sigil, gRPC, vendor SDKs, provider modules, or other third-party logging libraries.
...
- **AND** the dependency graph SHALL NOT include Sigil, OpenTelemetry SDKs, gRPC, vendor provider SDKs, or `github.com/grafana/ai-sdk/providers/*`
...
The package SHALL expose a `Redactor` interface, a `RedactorFunc` adapter, and `DefaultRedactor() Redactor`.

`Options.Logger == nil` SHALL default to `slog.Default()`. `Options.Level == nil` SHALL default to `slog.LevelInfo`. `Options.ErrorLevel == nil` SHALL default to `slog.LevelError`. `Options.Redactor == nil` SHALL default to `DefaultRedactor()`. `Options.Clock == nil` SHALL default to `time.Now`.
...
Every log record SHALL use the event string as the `slog` message and SHALL include `ai_sdk.event` with the same value.

Common records SHALL include stable `ai_sdk.*` attributes for call type, provider, model, and caller-provided static/dynamic attrs. Terminal records SHALL include duration in milliseconds and success/failure. Success records SHALL include available finish reason, token usage, warning count, and response metadata. Stream terminal records SHALL include total part count and per-type counts.
...
- **AND** it SHALL include `ai_sdk.call.type="generate"`, `ai_sdk.provider`, and `ai_sdk.model`
...
By default, the logger MAY log safe scalar summaries and counts, including provider/model identity, duration, success/failure, max output tokens, temperature, top-p, top-k, seed, reasoning effort value, stop sequence count, tool count, response format type, usage totals, finish reason, warning count, response metadata, and stream part counts.
...
- **THEN** the logged attribute value SHALL be truncated or summarized to stay within the configured or default bound
...
A custom `Redactor` SHALL receive the request context, event kind, and selected attrs immediately before logging.
...
4. On returned error, log `EventGenerateError` with duration, `ai_sdk.success=false`, sanitized error type/message, and API-call status/retryability when available; then return the original error unchanged.
5. On success, log `EventGenerateFinish` with duration, `ai_sdk.success=true`, finish reason, usage, warning count, response metadata, and opted-in captured response fields; then return the original `*provider.GenerateResult` unchanged.
...
6. Observe stream parts to accumulate counts, response metadata from `provider.PartResponseMeta`, usage/finish reason/provider metadata from `provider.PartFinish`, and the first `provider.PartError`.
7. Log `EventStreamPart` records only when `Options.LogStreamParts` is true.
8. Log exactly one terminal event when upstream closes: `EventStreamFinish` for successful completion or `EventStreamError` when a part error or context cancellation is observed.
...
- **AND** the terminal record SHALL be `EventStreamError` with `ai_sdk.success=false`
...
- **AND** it SHALL emit exactly one terminal `EventStreamError` record for the call
...
The logger middleware SHALL compose as an ordinary `middleware.Middleware` with `middleware.Wrap`, `middleware.WrapLanguageModel`, `registry.WithLanguageModelMiddleware`, fallback models, and `middleware/sigil`.
...
- that middleware ordering controls whether logs are outside or inside Sigil hooks/recording;
...
#### Scenario: Sigil ordering is caller controlled

- **WHEN** logger middleware is placed before Sigil middleware in the `middleware.Wrap` slice
- **THEN** logger SHALL be the outer middleware according to existing middleware ordering
- **AND** docs SHALL describe that this logs attempted calls including Sigil denials
