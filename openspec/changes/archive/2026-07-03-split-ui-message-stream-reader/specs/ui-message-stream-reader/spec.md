...
- **WHEN** `StreamUIMessage` receives file, source URL, source document, step start, non-transient data, or message metadata chunks
- **THEN** the output channel yields snapshots that include the corresponding Go message part or metadata update whenever upstream `ai@7.0.11` would write a message snapshot for that chunk type
