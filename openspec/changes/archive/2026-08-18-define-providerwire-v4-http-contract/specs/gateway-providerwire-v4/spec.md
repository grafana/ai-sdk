...
The fixture index SHALL identify regenerated request captures, curated response projections, and curated negative fixtures. It SHALL record their authority, update command, normalization, claims, and non-claims without duplicating baseline versions. Authorization values, credentials, volatile identifiers, machine-local paths, and provider-recording claims SHALL not be committed.

Response projections SHALL cover unary JSON, EOF-terminated SSE, tolerated `[DONE]`, raw filtering, timestamp conversion, safe errors, and retryability inference. They SHALL be labeled local projections rather than server captures.
...
- **WHEN** the ordinary contract interop command runs, even with an ambient update variable
- **THEN** it SHALL recapture semantically without rewriting committed fixtures

#### Scenario: Ownership is explicit
- **WHEN** a reviewer inspects the fixture index or GitHub diff
- **THEN** regenerated captures SHALL be distinguished from curated schemas, corpora, and projections

#### Scenario: Response projections prove consumption only
- **WHEN** the pinned client consumes a local projection
...
`mise run validate-providerwire-v4-contract` SHALL validate OpenAPI, offline references, strict syntax, HTTP envelopes, schema compilation, positive payloads, and declared negative failures. `mise run test-interop-contract` SHALL validate baseline pins, type-check capture tooling, verify request captures, and consume response projections. Capture replacement SHALL require a separate explicit task.

The parity map SHALL classify V4 contract evidence separately from legacy transport. A baseline change SHALL update the manifest, dependency pins, required source-equivalence evidence, schemas, captures, projections, negative corpus, parity classification, and lockfiles together.

#### Scenario: Contract validation is one command
- **WHEN** a contributor runs `mise run validate-providerwire-v4-contract`
- **THEN** every machine-readable contract and local validation corpus SHALL be checked
