...
- [x] 1.1 Create private `ai-gateway/test/providerwire-v4` package and TypeScript configuration with exact registered `@ai-sdk/gateway@4.0.52`, `@ai-sdk/provider@4.0.7`, and `@ai-sdk/provider-utils@5.0.27` dependencies plus the minimal locked test/schema tooling.
...
- [x] 2.1 Add the hand-authored draft 2020-12 schema at `ai-gateway/providerwire/v4/schema/request.json` with a closed root, required prompt, scalar settings, response formats, body headers, reasoning, provider options, tools, tool choice, and every registered role/content/data/result/approval union.
...
- [x] 5.2 Extend parity baseline validation and its tests to enumerate `ai-gateway/test/providerwire-v4/package.json` and report package/version drift for that workspace.
...
- [x] 5.5 Verify current documentation and package references distinguish strict `ai-gateway/providerwire/v4` artifacts from the retired unversioned handler/client and do not make a private Vercel server compatibility claim.
- [x] 5.6 Add a blocking `verify-ai-gateway-boundary` check that rejects reverse Go imports/module references, rejects Gateway workspace registration, and builds/tests the root SDK with `GOWORK=off`.
