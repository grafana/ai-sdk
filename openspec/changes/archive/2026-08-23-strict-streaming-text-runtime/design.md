...
Phase 3 added one strict `gateway/providerwire/v4` handler for unary text. It validates the exact ProviderWire envelope, bounds the body, applies standard Go JSON semantics and the complete request schema, maps the supported text subset through typed DTOs, resolves a canonical model, invokes `DoGenerate`, and commits only a bounded minimal private response DTO. Its envelope currently rejects `ai-language-model-streaming: true`.
