...
Phase 3 added one strict `gateway/providerwire/v4` handler for unary text. It validates the exact ProviderWire envelope, bounds and lexically validates JSON, applies the complete request schema, maps the supported text subset, applies policy, resolves a canonical model, invokes `DoGenerate`, and commits only a bounded schema-valid private response DTO. Its envelope currently rejects `ai-language-model-streaming: true`.
...
Envelope validation will return an execution mode after accepting only exact `false` or `true`. Both modes will share body, lexical, schema, mapper, policy, and resolver stages. Only the final model invocation and response path diverge. The mapper will continue to execute the same text/scalar subset in both modes; later mode-specific capabilities can branch explicitly when they land.

This preserves stage ordering and prevents a second streaming decoder from drifting from unary validation. It is a parity-preserving Go adaptation: HTTP mode dispatch differs structurally from TypeScript but preserves the registered request semantics.
...
The public stream always begins with exactly one server-owned start. If the first provider part is `PartStreamStart`, the runtime consumes it and maps all registered warning variants through one mapper shared with unary output. The mapper never copies arbitrary `Feature`, `Setting`, `Message`, or `Details` strings. Phase 4 uses this closed table and includes no model identity in warning prose:
...
Private DTOs will cover start, response metadata, text start/delta/end, finish, and safe error. A closed draft 2020-12 schema will validate each bounded JSON event. The existing bounded document writer will be extended or reused so JSON string escaping stops at `limit+1`; it will not `json.Marshal` an arbitrarily large provider value before checking size. Framing happens only after the complete JSON payload fits the remaining budget.
...
- replacing unary and streaming provider warning strings with fixed public prose: intentional privacy deviation from upstream passthrough, documented in `test/conformance/PARITY.md`;
...
- [Strict error objects are accepted but not validated by the client] → Make embedded schemas and raw HTTP assertions authoritative.

## Migration Plan

1. Extend handler construction with stream-part cardinality, mode parsing, shared safe warning mapping, and test helpers while keeping all unary tests passing.
...
3. Add private stream DTOs, event schema, bounded frame encoding, response-controller flushing, and raw encoder tests.
