...
- Extend the strict ProviderWire V4 handler to dispatch `ai-language-model-streaming: true` requests through `DoStream` after the existing envelope, body, schema, mapping, policy, and catalog stages.
...
- Normalize an optional provider `stream-start` into exactly one public start event using a shared value-safe warning mapper for unary and streaming output; align Anthropic warning timing with the pinned upstream provider by moving warnings from `finish` to start after the existing initial-event preflight.
...
- `providerwire-v4-unary-runtime`: Extend the constructed production handler and golden replay matrix to dispatch valid streaming envelopes, add the stream-part limit, and tighten unary warning values through the shared safe mapper while preserving all other unary behavior.
...
- Primary code: `gateway/providerwire/v4`, new embedded stream-event schemas, handler dispatch, stream invocation/state/framing code, shared bounded response helpers, and Anthropic stream-start warning emission.
