...
- Primary code: `gateway/providerwire/v4`, a test-only stream-event schema, handler dispatch, stream invocation/state/framing code, fixed precommit and terminal errors, shared bounded encoding helpers, and Anthropic stream-start warning emission.
