...
- Primary code: AGPL-owned `ai-gateway/providerwire/v4` for the stream-event schema, handler dispatch, stream invocation/state/framing, fixed precommit and terminal errors, and stream-owned bounded framing; Apache-owned `providers/anthropic` for reusable post-preflight stream-start warning emission.
