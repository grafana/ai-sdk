...
- Add race-safe stream setup ownership and commitment behavior using an unbuffered outcome transfer: every returned non-nil channel has one cleanup owner, setup failures and nil streams remain bounded non-2xx JSON, and a successfully transferred non-nil stream commits `text/event-stream`.
...
- Add standard Go JSON encoding with bounded dynamic-value preflight, exact complete-frame checks, and full writes through response-controller flushing; emit clean EOF immediately after authoritative finish, never emit `[DONE]`, and synthesize at most one safe terminal adapter error before finish.
- Add standard context and timer cancellation, provider-work cancellation, writer-failure termination, race-tolerant terminal outcomes, and bounded asynchronous channel drain behavior.
...
- Primary code: AGPL-owned `ai-gateway/providerwire/v4` for the stream-event schema, handler dispatch, stream invocation/state/framing, fixed precommit and terminal errors, and standard JSON frame encoding; Apache-owned `providers/anthropic` for reusable post-preflight stream-start warning emission.
- Tests: ProviderWire V4 Go runtime tests and co-located golden replay, testserver, and pinned-client integration under `ai-gateway/test/providerwire-v4`, plus focused Anthropic provider tests against unchanged provenance-valid inputs.
