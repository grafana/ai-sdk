...
The strict ProviderWire V4 handler SHALL require positive limits for provider stream-part count, complete SSE frame bytes, stream idle duration, and post-cancellation drain duration in addition to the existing request, unary, error, and total model-duration limits. Construction SHALL reject a frame limit that cannot safely use `limit+1` or cannot contain both the canonical empty `stream-start` frame and canonical terminal internal-error frame. Complete frame size SHALL include the `data: ` prefix, JSON payload, and terminating `\n\n`.

#### Scenario: Valid streaming limits
- **WHEN** a caller supplies positive part-count, safe frame, idle, and drain limits whose frame limit contains both canonical fallback frames
...
- **WHEN** a streaming limit is zero or negative, the frame limit cannot safely use `limit+1`, or the frame limit cannot contain either canonical fallback frame
- **THEN** construction SHALL fail before the handler serves a request

### Requirement: Shared strict streaming request pipeline
The handler SHALL accept `ai-language-model-streaming` only when its single exact value is `true` or `false`. It SHALL route `false` to the existing unary path and `true` to the streaming path only after the same bounded body read, lexical validation, complete request-schema validation, explicit text-subset mapping, host policy, exact-once catalog resolution, and canonical-resolution validation. A supported streaming request SHALL invoke `DoStream` exactly once and SHALL NOT invoke `DoGenerate`. Any failure before stream invocation SHALL retain the existing bounded non-2xx JSON behavior and SHALL produce no SSE commitment.

#### Scenario: Supported streaming envelope executes once
- **WHEN** a valid text request uses streaming value `true` and passes mapping, policy, and resolution
- **THEN** policy and resolution SHALL each run once in order, `DoStream` SHALL run once, and `DoGenerate` SHALL not run

#### Scenario: Streaming request fails before invocation
- **WHEN** a streaming request fails envelope, body, lexical, schema, mapping, policy, or resolution processing
- **THEN** no later stage SHALL run and the response SHALL remain a bounded non-2xx JSON error rather than SSE
...
- **THEN** envelope validation SHALL fail before body mapping, policy, resolution, or model invocation
