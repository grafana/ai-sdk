...
- [x] 3.3 Implement bounded `data: <json>\n\n` encoding that counts the full frame and never first accumulates an oversized provider value; keep schema validation in tests rather than the production write path.
...
- [x] 9.2 Run `(cd ai-gateway && GOWORK=off go test -race ./providerwire/v4)` to validate setup ownership, shared clocks and timers, part counters, and asynchronous drain cleanup.
