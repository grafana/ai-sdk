## 1. Handler Mode and Limits

- [x] 1.1 Add failing construction tests for positive provider stream-part count, complete-frame, idle, and drain limits, including canonical start/error frame fit.
- [x] 1.2 Extend `v4.Limits` and validation for `StreamParts` and the other streaming limits while preserving phase 3's request, unary, and fixed-error behavior.
- [x] 1.3 Add failing envelope and sequencing tests for exact `true`/`false` mode selection, invalid streaming values, the shared standard-JSON/schema/mapping path, and `DoGenerate` versus `DoStream` dispatch.
- [x] 1.4 Refactor request validation and handler dispatch to return execution mode, reject invalid canonical resolved IDs before model invocation, and route supported text requests through the correct model method without duplicating the strict pipeline.

## 2. Shared Safe Warnings and Anthropic Timing

- [x] 2.1 Preserve phase 3's minimal unary response and add streaming warning privacy tests covering hostile credentials, URLs, bodies, headers, provider/backend identity, arbitrary prose, empty values, unknown discriminators, and cardinality rejection before mapped-slice allocation.
- [x] 2.2 Implement a streaming-specific value-safe warning mapper that emits only closed identifiers and fixed approved prose.
- [x] 2.3 Keep unary response tests and raw expectations limited to content, finish reason, usage, privacy, and byte bounds.
- [x] 2.4 Add Anthropic tests proving initial API failures remain `DoStream` setup errors and successful preflight emits one `PartStreamStart` before handling/emitting the pre-read event, with warnings absent from `PartFinish`.
- [x] 2.5 Move Anthropic streaming warnings to the post-preflight initial provider start while preserving all later event order, initial-error promotion, and finish values.
- [x] 2.6 Run `mise run test-anthropic`; regenerate derived conformance expectations only if unchanged provenance-valid inputs are affected, and verify no `test/conformance/**/input*.chunks.txt` file changed.

## 3. Private Stream DTOs and Bounded Framing

- [x] 3.1 Add a test-only closed draft 2020-12 stream-event schema and positive/negative tests for normalized start warnings, canonical metadata, text events, finish, and the closed safe error object.
- [x] 3.2 Add private stream DTO mapping and standard JSON complete-frame encoding tests covering required empty values, invalid UTF-8, exact below/at/above frame boundaries, warning cardinality prechecks, and canonical fallback frames.
- [x] 3.3 Implement dynamic-value preflight, standard Go JSON encoding, and an exact `data: <json>\n\n` frame check before writing; keep schema validation in tests rather than the production write path.
- [x] 3.4 Add response-writer tests for full and short writes, commitment-header and per-frame `http.NewResponseController(w).Flush()`, direct and wrapped `http.ErrNotSupported`, separate header/frame flush failures, panic recovery, exact SSE headers, no `event:` field, and no `[DONE]`.
- [x] 3.5 Implement identical header/frame flush classification with `errors.Is(err, http.ErrNotSupported)` so unsupported flushing is tolerated and every other observable writer failure cancels provider work without a second write.

## 4. Setup Ownership and Commitment

- [x] 4.1 Add setup tests for provider errors, panic, `nil, nil`, nil channel, result-plus-error with a channel, private `StreamResult` metadata, valid commitment, and result-plus-error JSON latency independent of drain duration.
- [x] 4.2 Add ownership tests for setup success, cancellation, timeout, late outcomes, and asynchronous drain without requiring scheduler-level race precedence.
- [x] 4.3 Implement unbuffered setup outcome transfer so the handler owns received streams and the worker owns late streams after model-context termination.
- [x] 4.4 Implement valid-result commitment and asynchronous owner cleanup for every invalid, result-plus-error, late, or transferred non-nil channel.
- [x] 4.5 Add and implement no-first-part behavior so established streams remain SSE and attempt empty start plus the applicable terminal error after cancellation or timeout.

## 5. Start, Cardinality, Metadata, and Text State

- [x] 5.1 Add start tests for provider start present/absent, normalized known and generic warning variants, hostile warning privacy, duplicate/late start, oversized warnings, and warning-count rejection before allocation.
- [x] 5.2 Implement first-part pre-read and exactly-one public start normalization through the streaming-specific safe warning mapper, including empty start plus terminal fallback when warnings cannot be represented.
- [x] 5.3 Add below/at/above `StreamParts` tests counting start, metadata, errors, text, and finish, plus a continuously ready small-block flood that proves bounded retained ID state without timeout scheduling.
- [x] 5.4 Implement pre-interpretation provider-part counting so the first excess part cannot mutate lifecycle state, grow the ID set, or emit provider-derived output.
- [x] 5.5 Add table-driven transition tests for canonical metadata/privacy, metadata duplication/placement, sequential text blocks, empty deltas, active-ID matching, empty/invalid/reused IDs, and overlapping blocks.
- [x] 5.6 Implement the closed text state machine with canonical response metadata, globally unique non-empty UTF-8 text IDs bounded by `StreamParts`, one active block, ordered events, and required empty delta preservation.
- [x] 5.7 Add unsupported-family and hostile provider-metadata tests proving reasoning, tools, approvals, files, sources, custom, raw, and provider metadata terminate safely without provider-domain serialization.

## 6. Provider Errors and Finish Authority

- [x] 6.1 Add tests for one and multiple provider errors before/between/within text blocks, later content and finish, nil/malformed errors, closed status/retryability fields, and hostile-detail privacy.
- [x] 6.2 Implement independent non-terminal `PartError` reduction through the existing safe-category table without changing stream lifecycle state.
- [x] 6.3 Add finish tests for finish-only and text streams, normalized usage and reasons, invalid/unsafe usage, active blocks, finish warnings, premature EOF, duplicate finish, and post-finish provider floods.
- [x] 6.4 Implement finish validation as the authoritative final public event, immediate clean EOF without waiting for channel close, premature-EOF synthetic failure, and post-finish suppression during asynchronous drain.
- [x] 6.5 Add tests proving lifecycle, mapping, unsupported-output, part-limit, and frame failures emit at most one synthetic terminal adapter error without synthetic block ends or finish.

## 7. Standard Timeouts and Bounded Drain

- [x] 7.1 Use a request-derived `context.WithTimeout` for total model duration and leave scheduler-level precedence unspecified when output, cancellation, and timeouts race.
- [x] 7.2 Use one resettable standard timer for stream idleness and cancel provider work before timeout output.
- [x] 7.3 Add focused tests proving total and idle timeout outcomes, provider cancellation before output, finish authority, and idle reset after accepted activity.
- [x] 7.4 Add drain tests for normal close, delayed close, result-plus-error streams, late setup, post-finish values, and pre-commit JSON/handler latency independent of drain duration.
- [x] 7.5 Add a permanently ready flood-channel drain test proving termination within the configured duration or at the first excess request-scoped provider part.
- [x] 7.6 Implement one asynchronous drain per outcome owner using standard time and the shared provider-part counter.

## 8. Contract Replay and Cross-Language Evidence

- [x] 8.1 Update Go golden replay tests so `streaming.json` and the streaming `sequence.json` record execute `DoStream` once with exact mapped options while all unary records retain their intended outcomes.
- [x] 8.2 Add raw production-handler SSE tests for headers, exact frames, safe warning values, canonical metadata, empty/non-empty deltas, ordered provider errors, finish, immediate clean EOF, privacy, schemas, part limits, and frame limits.
- [x] 8.3 Extend the AGPL `ai-gateway/test/providerwire-v4` runtime integration with un-aborted pinned-client scenarios against the production Go handler for normal streams, multiple ordered provider errors followed by content, adapter timeout, finish, and clean EOF without `[DONE]`.
- [x] 8.4 Add a separate established-stream abort scenario: return a non-nil silent channel, await pinned-client `doStream()` resolution and HTTP commitment, then abort and assert server-side recording-provider context cancellation.
- [x] 8.5 Extend the deterministic co-located `ai-gateway/test/providerwire-v4/testserver` and matching registered-client coverage for ProviderWire SSE behavior crossing the frontend wire boundary; do not create synthetic provider conformance input.
- [x] 8.6 Update `test/conformance/PARITY.md` to classify fixed-prose streaming warning normalization as an intentional privacy deviation, preserve minimal unary behavior, classify strict streaming text evidence accurately, and retain explicit gaps for every deferred stream family and permissive-client evidence boundary.

## 9. Validation

- [x] 9.1 Run `mise run fmt`, `(cd ai-gateway && GOWORK=off go test ./providerwire/v4)`, and `mise run test-anthropic`; fix all focused failures.
- [x] 9.2 Run `(cd ai-gateway && GOWORK=off go test -race ./providerwire/v4)` to validate setup ownership transfer, standard timeout behavior, part counters, and asynchronous drain cleanup.
- [x] 9.3 Run `mise run test-providerwire-v4` and `mise run test-integration`; verify normal ProviderWire checks do not rewrite tracked goldens.
- [x] 9.4 Run `mise run parity-check` and inspect every conformance expectation diff for matching registered-baseline behavior and valid input provenance.
- [x] 9.5 Run `mise run build` and `mise run test` across all modules and examples.
- [x] 9.6 Run `mise run vet`, `mise run lint`, and `mise run test-short`; remove debug code, unused imports, and temporary files.
- [x] 9.7 Run `openspec validate --all --strict`, review the final diff against the revised phase 4 authority and upstream commit `d76eb85a9a7f2dbe44ab2f3dc858ad5cdcb5242e`, and document residual provider lifecycle risks.
- [x] 9.8 Publish the Apache Anthropic prerequisite on an immutable branch, pin its public-proxy SDK pseudo-version from `ai-gateway/go.mod` without replacements, and verify the AGPL module and one-way dependency boundary.
