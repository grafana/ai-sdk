...
- [x] 8.3 Extend the general integration suite with un-aborted pinned-client scenarios against the production Go handler for normal streams, multiple ordered provider errors followed by content, adapter timeout, finish, and clean EOF without `[DONE]`; keep `test/providerwire-v4` focused on contract/client probes.
