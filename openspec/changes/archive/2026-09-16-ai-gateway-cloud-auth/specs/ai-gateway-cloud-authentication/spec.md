...
The Gateway-owned guide SHALL document the application's required identity headers and proxy-only API access prerequisite. The proxy MUST authenticate and authorize requests, remove client credentials, and replace client-supplied identity headers before forwarding. The application SHALL NOT verify proxy credentials or repeat access-policy checks.
...
- **THEN** it documents application settings, required headers, isolation, and client limitations
...
Cloud mode MUST validate trusted identity headers without verifying CAP tokens or checking CAP scopes. It MUST require exactly one value for `X-Scope-OrgID`, `X-Cloud-Org-ID`, and `X-Access-Policy-ID`. Stack and organization IDs MUST contain positive decimal digits fitting `int64`. The policy identifier MUST be nonempty and opaque, without a UUID requirement.

Validation SHALL reuse `exactlyOneHeader` and reject missing, empty, duplicated, case-colliding, comma-coalesced, control-character, and invalid-whitespace assertions. The application SHALL NOT normalize malformed assertions into valid credentials.

`Caller` MUST retain a typed authentication source and distinct private stack, policy organization, and policy identifier fields. It MUST derive the namespace from the stack through the pinned `types.CloudNamespaceFormatter`. It MUST NOT substitute the policy ID for `Caller.Service`, treat the policy organization as the target stack, or manufacture a service identity or acting user.
...
#### Scenario: Opaque policy identifier

- **WHEN** the application receives valid proxy assertions with an opaque non-UUID policy identifier and no client credential
- **THEN** it accepts the opaque identifier and derives the namespace through `types.CloudNamespaceFormatter`
- **AND** it retains the stack ID, policy organization ID, and policy identifier separately
- **AND** it does not manufacture a service identity or acting user
- **AND** it makes no outgoing authentication request

#### Scenario: Invalid post-edge assertion set

- **WHEN** a controlled fixture injects missing, empty, duplicated, case-colliding, comma-coalesced, control-character, or invalid-whitespace assertions after shim header replacement
...
- **WHEN** a stack or policy organization assertion contains zero, a sign, nondigits, or a value exceeding `int64`
...
The registered low-level Gateway client MUST work through a test-only edge shim with the real command. The shim MUST use fixed dummy credentials and scope outcomes, not production CAP verification or policy evaluation. It SHALL strip Cloud and internal credentials, overwrite all three trusted assertions, and forward the unchanged path.
...
| Spoofed client assertions with valid dummy credentials | The application receives the shim's identity values, not client-supplied values. |
