...
### Refuse a content field, not only the part fields

Review found that `content` reaches the backend as a part array, so an image or file part travels underneath the checks on `type` and `image_url` and reinstates what the runtime refused to map. Reproduced against a local backend: a message-level `content` put an `image_url` part on the wire, and a part-level `content` rewrote the prompt the caller was billed for. Both are refused now.
...
### One policy per model, even with fallbacks

Ordered fallback lets one model resolve to candidates from different provider types, while the catalog carries one policy. Where the candidates agree, that policy applies. Where they do not, the model forwards no caller provider options, because forwarding the primary's policy would hand one backend's fields to another. Making this per attempt needs the policy to travel with the candidate, which belongs with the fallback work.
