...
- Refuse provider-option fields that name a decision the runtime already made, in any spelling a provider reads.
...
- `ai-gateway/cmd/grafana-ai-gateway`: per-provider policies in `internal/service`, and the reserved `providerName` check in `internal/config`.
