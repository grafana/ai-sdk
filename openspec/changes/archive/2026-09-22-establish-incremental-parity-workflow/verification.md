...
## Issue registration and labels

Read-only GitHub queries verified the actual label inventory and sampled existing open/closed upstream-sync issues. The reference now specifies actionable-package eligibility, labeled/unlabeled semantic searches, candidate/resolution inspection, reuse/overlap decisions, issue contents and label selection. Every created or reused parity work-package issue requires upstream-sync; existing optional bug/enhancement/documentation/question labels apply by work type. Unrelated workflow/triage labels and public security disclosure are not inferred.

The local daily prompt follows the same rules and explicitly requires verifying the mandatory label. Its prior version is backed up at `/home/nara/.local/share/ai-sdk-artifacts/parity-issue-registration-qiTuf8`. Repo docs/Markdown lint, links/anchor, strict OpenSpec, whitespace and Herdr configuration validation passed, as did a placeholder-credential dry-run without launch. No live issue was created, edited, labeled, closed or reopened during this instruction update. Runtime tooling and canonical baseline assets remain unchanged.

## Finalization

On 2026-09-22, the operator approved spec sync and archive. Two governance requirements were updated and seven added; a requirement-level comparison confirmed the delta was fully represented and unrelated requirements were preserved. All 67 main specs passed strict validation. The change was archived with all tasks complete, and the repository now has no active OpenSpec changes. The separate upgrade-test worktree was not modified.
