...
The repository has eleven published Go modules (root, Gateway, providers, middleware), a root `go.work` for SDK modules and local-only test/example modules, and a standalone public-proxy gate (`mise run verify-module-resolution`). The original boundary script coupled structural AGPL/Apache checks with root, Grafana client, and Gateway standalone builds/tests. Gateway command integration normally builds with `GOWORK=off`; other test builds select root-workspace or isolated behavior inconsistently. Some committed internal pseudo-version requirements are branch-only. The registered Vercel AI SDK baseline is `test/conformance/upstream.yaml`; this change affects build governance, not upstream behavior.
...
Consolidate module checks under `scripts/module-policy.sh`; its boundary mode verifies licenses and Gateway module identity, keeps Gateway and replacements out of root `go.work`, rejects Gateway references in tracked non-Gateway Go files and module manifests (including nested modules), and checks root SDK and Grafana client module graphs. Treat `ai-gateway/` as the AGPL directory boundary; the shared module inventory validates tracked module paths. Do not build/test again inside the structural check. The existing all-published-modules gate already builds/tests the SDK root and Grafana client with `GOWORK=off`, a fresh public-proxy cache, readonly manifests, and no published-module replacements. Issue #244 also requires a literal source-absent proof: build/test the SDK and Grafana client from a temporary copy excluding Gateway, without a separate script. Add a module-root/path-selectable standalone entry point using the same gate. Unknown or local-only selections fail. One Bash policy entry point shares module discovery and exposes standalone, boundary, ancestry, workspace, and source-absent checks. Deterministic Bash fixtures test the Git/proxy boundary and candidate-source selection. The existing `module-resolution` CI job continues calling the all-module and boundary gates, plus the ancestry guard after migration. Do not weaken image validation or publication/deployment dependencies.
