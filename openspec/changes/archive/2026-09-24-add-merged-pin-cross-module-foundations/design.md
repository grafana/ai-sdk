...
Refactor `verify-ai-gateway-boundary.sh` to enforce license, module ownership, root-workspace exclusion, imports/requires/replaces, and root graph exclusions without invoking public-proxy build/test. Analyze Go source by its nearest tracked `go.mod` owner (including nested modules), so checks exclude only Gateway-owned files, not arbitrary directory subtrees or the parent of a nested module. Preserve a separate proof that root and Grafana client build/test with Gateway source unavailable and `GOWORK=off`; running without a Gateway directory is stronger than only checking imports. Keep the existing all-published-modules entry point and add a module-root/path-selectable standalone entry point backed by the same isolated public-proxy, fresh-cache, readonly, no-replace checks. Unknown or local-only module selections fail. The existing `module-resolution` CI job continues calling the all-module and boundary gates, plus the ancestry guard after migration. Do not weaken image validation or publication/deployment dependencies.
...
- [Nested modules or test placeholders classified incorrectly] → Derive ownership from tracked module roots and declared paths; cover nested ownership, real dependency graph, and deliberate local-only replacements in tests.
