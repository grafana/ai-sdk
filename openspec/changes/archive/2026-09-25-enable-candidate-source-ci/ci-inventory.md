...
| `module-resolution`: `verify-module-resolution` | all published modules, public proxy, fresh cache, readonly `GOWORK=off` build/test | remove from required CI; keep on-demand selected/all-module entry points, with Gateway selected at artifact validation |
...
| `image-validation` | Dockerfile builds Gateway standalone `GOWORK=off`, multiarch + native smoke | verify same-revision Gateway standalone first, then multiarch/native image and smoke; push-only |
| `publish-ai-gateway-image` | eligible canonical main/tag push needs source jobs and image-validation; Dockerfile standalone | depend on required source jobs and successful push-only image validation |
