...
Non-goals: MCP server configuration or MCP metadata, later content families, local tool execution, persistent sessions, fallback effects or an upstream baseline upgrade.
...
- Bound metadata before projection. Anthropic caller type/toolId and OpenAI/Azure itemId, namespace and caller type/callerId are reviewed producer/replay fields; unknown private fields are omitted. Explicit MCP metadata fails rather than becoming an ordinary tool. Ordinary request-part options follow the existing protected-field and configured-backend policy, including tool call/result parts.
- Keep MCP acceptance out of this PR rather than adding a disabled feature switch. Preserve safe root, message and part options and body-carried headers already supported by the Gateway; protect host-owned fields at each level. The successor introduces a narrowly authorized MCP option, configured-server matching and service-owned route eligibility together.
...
- Incomplete intermediate evidence → both registered clients exercise native code execution and deferred-result handler scenarios without MCP; candidate-source, merged-pin ancestry and full parity gates run at this branch.

## Migration Plan

Enable after SDK prerequisites, then add hosted MCP separately. Each capability retains scoped docs and an unarchived OpenSpec change; internal pins remain on canonical-main revisions until publication. There is no persisted-state migration; rollback uses the prior image/module set. Deployment activation is unverified.
