...
- Populate existing provider result fields with SDK-submitted request JSON and actual HTTP response headers for Anthropic and OpenAI, in generate and stream modes. Preserve unary response JSON body where the existing contract supports it; do not add a body to streaming responses.
- Emit `PartRaw` only when `IncludeRawChunks` is true, before normalized or error parts for observable decoded events; an observable syntax-invalid event has an absent raw value. Intercept Anthropic SSE frames at the call-local response-body boundary before its SDK decoder hides them, while retaining SDK ownership and preflight failures; make cancellation release the stream even when the consumer stalls. SDK-filtered malformed ping/unknown-event parser errors remain a documented limited-support boundary: raw frame values are visible, but the adapter does not synthesize normalized errors absent from the SDK.
