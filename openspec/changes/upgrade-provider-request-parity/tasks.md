...
- [ ] 10.3 Verify implementation against these artifacts and archive the single active change before merge. Verification is summarized in `evidence.md`; the detailed report is preserved externally, and archival is the remaining before-merge action. Land independently only if enforced checks pass with documented ahead-of-baseline behavior; otherwise preserve scoped review for the plan's validated cumulative integration landing.
