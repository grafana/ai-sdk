...
Define public model ID enumeration and Vertex model ID resolution helpers for the `providers/anthropic` module's `anthropic` package.

## Requirements

### Requirement: Direct Anthropic model ID enumeration
The `providers/anthropic` module's `anthropic` package SHALL export `ModelIDs() []string`, returning the package's curated list of model IDs for the direct Anthropic API in the form accepted by `New`.
...
The `providers/anthropic` module's `anthropic` package SHALL export `VertexModelIDs() []string`, returning the package's curated list of Anthropic model IDs for the Vertex AI partner channel in the resolved Vertex form expected by `NewVertex` requests.
...
The `providers/anthropic` module's `anthropic` package SHALL export `DualAvailableModelIDs() []string`, returning curated model IDs that are available on both the direct Anthropic API and Vertex AI partner channel.
