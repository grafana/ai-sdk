# anthropic-prompt-conversion Specification

## Purpose

Provider-side conversion semantics from the V4 `provider.Message` /
`provider.ContentPart` shape to Anthropic's Messages API request shape.
Captures the rules that govern message grouping (consecutive `user`/`tool`
merge into one Anthropic user block, consecutive `assistant` merge into one
Anthropic assistant block, consecutive `system` merge into the prompt-level
`system` array), per-role part dispatch (which `ContentPartType` is valid in
which role), and the per-source-message cache-control cascade across merged
blocks. This capability codifies the upstream `convertToAnthropicPrompt` +
`groupIntoBlocks` contract for the Go port.
...
- **THEN** the resulting Anthropic assistant message SHALL contain `OfText{Text: "Final prefill"}`

#### Scenario: Earlier assistant text preserves whitespace
- **WHEN** an assistant text part containing `"Earlier text  \n"` is followed by another prompt block or by another text part in the final assistant message
- **THEN** that earlier text SHALL remain exactly `"Earlier text  \n"` in the Anthropic request

#### Scenario: Text before a final non-text part preserves whitespace
- **WHEN** the final assistant message has content `[text("Calling tool  \n"), tool_call("c1", "search", {})]`
- **THEN** the resulting text block SHALL remain `OfText{Text: "Calling tool  \n"}`

#### Scenario: ECMAScript whitespace semantics are preserved
- **WHEN** the final ordinary assistant text is surrounded by U+FEFF byte order marks
- **THEN** the resulting text SHALL omit those marks, matching JavaScript `trim()`
- **AND WHEN** it is surrounded by U+0085 next-line characters
- **THEN** those characters SHALL remain, because JavaScript `trim()` does not remove them
...
The `provider.LanguageModel` interface, the `BuildParams` signature, and the SSE chunk types emitted by the orchestration layer SHALL remain unchanged. Apart from the prompt-conversion behaviors specified above, Anthropic request serialization SHALL remain byte-identical for unaffected prompts.

#### Scenario: Unaffected prompts produce identical wire requests
- **WHEN** a prompt does not exercise grouping, tool-result-in-user-message conversion, final assistant prefill whitespace normalization, or assistant compaction serialization
