# gateway-error-normalization Specification

## Purpose

Define a normalized gateway error type and the normalizer that maps structured provider errors into a stable category vocabulary, mirroring the `@ai-sdk/gateway` `GatewayError` contract while preserving the originating `APICallError` as cause.

## Requirements

### Requirement: Normalized gateway error with type discriminator

The Grafana gateway provider package (`providers/grafana`) SHALL export a
`GatewayError` type that carries a normalized category as a typed string
discriminator field `Type GatewayErrorType`, mirroring the `@ai-sdk/gateway`
`GatewayError.type` contract. Like upstream, this lives in the gateway provider
package (the analog of `@ai-sdk/gateway`), not in the core `provider` package.
`GatewayError` SHALL implement the `error` interface and expose `Error()` and
`Unwrap()`. The package SHALL define typed constants for the upstream category
vocabulary:
...
- **WHEN** a normalized `*GatewayError` is returned and `errors.As(err, &target)` is called with `target` of type `*grafana.GatewayError`
...
The Grafana gateway provider package SHALL expose a normalizer that converts a
`*provider.APICallError` into a `*GatewayError`, reading the structured provider error
