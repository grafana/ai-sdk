...
Define Anthropic MCP server configuration, streamed and non-streamed MCP tool behavior, and prompt round-tripping requirements.
...
- **THEN** the adapter emits a `PartToolResult` with `ToolCallID: "tc_123"`, `ToolName: "get_weather"`, `Dynamic: true`, `ProviderMetadata` from the tracked tool call, and `Result` containing the raw JSON bytes `[{"type":"text","text":"Result data"}]`
...
- **THEN** the `Result` field SHALL contain the exact JSON as received from the Anthropic API, not a re-serialization of the SDK's union type
...
- **THEN** the `Result` field SHALL contain the raw JSON string `"some result"`, not `{"OfString":"some result","OfBetaMCPToolResultBlockContent":null}`
...
- **THEN** the adapter emits a `PartToolResult` with `IsError: true`, `Dynamic: true`, and the error content as raw JSON in `Result`
