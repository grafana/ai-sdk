...
content part returns non-empty logprobs, `ProviderMetadata["openai"].logprobs`
SHALL contain one outer entry per such content part in response order. Each entry
SHALL preserve token order and contain `token`, `logprob`, and `top_logprobs`
alternatives with `token` and `logprob`, without provider-only byte arrays. Empty
or unrequested logprobs SHALL NOT add a `logprobs` metadata field.

#### Scenario: Generated output logprobs metadata
- **WHEN** logprobs are requested and output-text content returns non-empty token logprobs with top alternatives
...
- **WHEN** logprobs were not requested or every returned output-text logprobs array is empty
...
SHALL NOT error. When logprobs were requested, non-empty
`response.output_text.delta.logprobs` arrays SHALL be accumulated in event order
and included as `ProviderMetadata["openai"].logprobs` on the final `PartFinish`,
using the same normalized token and top-alternative shape as non-streaming
responses. Empty or unrequested stream logprobs SHALL NOT add a `logprobs`
metadata field.

#### Scenario: Stream finish with logprobs
- **WHEN** logprobs are requested and output-text delta events return non-empty token logprobs with top alternatives
...
- **WHEN** logprobs were not requested or every output-text delta logprobs array is empty
