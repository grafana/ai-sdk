...
The strict pinned V4 HTTP contract and any runtime built from it SHALL be owned by the sibling `gateway/providerwire/v4` capability. The legacy package SHALL remain independently available and behaviorally unchanged.
...
#### Scenario: Strict runtime ownership remains separate
- **WHEN** a strict V4 runtime is introduced
- **THEN** it SHALL remain under `gateway/providerwire/v4` and SHALL NOT redefine or replace the legacy package's exported codecs
