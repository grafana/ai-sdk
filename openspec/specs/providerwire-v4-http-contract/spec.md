...
The dependency boundary SHALL be one-way: Gateway code MAY import explicitly pinned SDK modules, but no Go source or module outside `ai-gateway/` SHALL import, require, or replace the Gateway module. The Gateway module SHALL remain absent from the root `go.work` and root module graph, and the root SDK SHALL build and test with `GOWORK=off`. A separately named, explicitly selected integration workspace MAY include Gateway and local SDK source for candidate-source testing; its existence SHALL NOT change the root workspace, permit a reverse SDK-to-Gateway dependency, or replace standalone publication/production validation.
...
- **WHEN** the separate Gateway source-integration workspace is selected for a coordinated source test
...
- **AND** release/production Gateway builds SHALL still use `GOWORK=off`
