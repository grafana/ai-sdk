...
Define retry eligibility, attempt budgets, backoff, cancellation, and exhaustion behavior for model calls.
...
#### Scenario: Retryable established-stream error is not replayed
- **WHEN** `DoStream` successfully returns a provider stream
- **AND** that established stream later emits a `PartError` whose `APICallError.IsRetryable` is true
- **THEN** `StreamText` SHALL surface the stream error without invoking `DoStream` again
- **AND** output or effects from the failed attempt SHALL NOT be duplicated by transparent replay
