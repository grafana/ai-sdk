...
observation are tested without a Gateway executor or session. All nonempty root
options and MCP metadata remain rejected; remote MCP activation is a separate
capability. No live-provider or deployment smoke is claimed.
...
- Intentional security boundaries: client-owned authentication/protocol headers
  ...
  and command evidence; MCP remains unsupported by the service.
