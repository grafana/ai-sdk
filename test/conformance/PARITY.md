...
| Message and content part taxonomy | mixed | Provider request snapshots cover current message/content variants. The Anthropic `ui-tool-model-output` fixture verifies that persisted UI tool results apply `Tool.ToModelOutput` and reach the provider as multipart text/file content. | New content parts must include provider request assertions. Known Go conversion gaps versus `ai@7.0.44` include upstream `convertDataPart` support and some tool conversion semantics (`input-streaming` filtering default and output-denied default text); preserve existing Go behavior unless a focused parity change updates fixtures and docs. |
...
| Capability | Status | Confidence Source | Gap / Notes |
| --- | --- | --- | --- |
| Anthropic request conversion | automated | `test/conformance/anthropic/**/expected-requests.jsonl` compares behavior-affecting requests. | Add fixtures for new Anthropic options, content parts, and beta headers. Intentional compatibility deviation from `@ai-sdk/anthropic@4.0.25`: with an empty input tool list, Go omits `auto` and `none` choices but preserves explicit `required` and named-tool choices; upstream omits every tool choice. |
| Anthropic response stream parsing | automated | Anthropic fixture replay compares Go stream output with upstream expectations; provider tests cover fallback iteration attribution, stop details, recommended-model metadata, and streamed finish metadata. | Newly observed event types need fixtures before release. |
...
| Provider-wire upstream encode compatibility | mixed | The legacy provider-wire transport has automated provider, provider-wire, conformance, and interop coverage. The independent `gateway/providerwire/v4` request codec uses private DTOs and focused Go tests for canonical call options, every supported request discriminator, tagged file data, strict active-arm validation, fail-fast unknown standard fields, inactive sibling-arm tolerance, opaque extension boundaries, typed nulls, explicit empty `tools` and `stopSequences`, privacy fields, provider options, and legacy-package isolation, with manual source comparison against the registered provider contract. | The strict codec is request-only and is not yet used by an HTTP boundary. It intentionally enforces semantic non-empty identifiers beyond TypeScript's structural string types. Rejecting unknown standard request fields before invocation is an intentional fail-fast service-boundary deviation from TypeScript's structural object behavior; it prevents a newer caller's unsupported intent from being silently discarded. Rejecting non-empty top-level `providerOptions.gateway` and every nested `gateway` namespace is also intentional: pinned Gateway clients can send routing, fallback, BYOK, caching, timeout, tier, attribution, and data-governance controls, but this request-only codec has no routing-policy layer and does not pass gateway controls to provider-visible options. A future host-owned policy layer may implement specific controls before provider invocation; until then they remain explicitly unsupported and require negative HTTP interoperability coverage when the strict route is added. Provider references reject the exact key `type` as required by `SharedV4ProviderReference`. Known inactive sibling-arm fields remain ignored, while provider options and defined opaque values remain open extension boundaries. |
...
| Capability | Status | Confidence Source | Gap / Notes |
| --- | --- | --- | --- |
| `@ai-sdk/react` `useChat` wire compatibility | automated | Integration tests consume Go UI message SSE through the upstream React hook package, including metadata-only text deltas and assembled invalid provider-tool error state. | Add scenarios for tool approval and multi-step UI state as risk increases. |
