...
inputs are unchanged. Gateway boolean consumers and immutable dependencies were
migrated separately before enabling provider tools.

Direct-route provider definitions and provider-owned unary/SSE results now have
pinned-client request captures, both-client differential and real-handler tests,
plus authenticated native Anthropic code-execution alias/continuation coverage.
History-aware deferred results, preview finalization, bounds and metadata-only
observation are tested without a Gateway executor or session.

The separate MCP extension has its own pinned-client root-option/continuation
golden and both-client deferred-result and authenticated native transport tests,
while provider-only cases remain. Only direct Anthropic routes accept validated
MCP options; configured names alone appear in normalized metadata. URLs/tokens
reach the selected native transport and caller-owned request, not normalized
output or operational telemetry. Fallback rejects nonempty MCP configuration
even without tools. All other nonempty root options remain unsupported. This
is explicit Grafana safety policy, not a claim about Vercel's private service.
No live MCP egress or deployment smoke is claimed.
...
- Intentional security boundaries: client-owned authentication/protocol headers
  ...
  result decoding. Gateway provider-tool activation has separate real-handler
  and command evidence; the separately tested MCP extension is limited to
  caller-configured servers on direct Anthropic routes.
