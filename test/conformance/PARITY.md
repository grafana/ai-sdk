...
## September 2026 target assessment

The fixed target is `ai@7.0.107`, Anthropic `4.0.58`, Bedrock `5.0.88`, OpenAI
`4.0.71`, OpenAI-compatible `3.0.53`, Gateway `4.0.87`, provider-utils `5.0.45`,
React `4.0.110`, and provider `4.0.17`. Source paths below use exact
[08ae5ad0](https://github.com/vercel/ai/tree/08ae5ad05bc12496dd1ffcf64e34419e0831300d/packages),
except provider source, which uses
[1a82df1c](https://github.com/vercel/ai/tree/1a82df1c7b6239f4cefe9fb73c357b3a6529c629/packages/provider/src).
Linked packages contain source/test references, impact, design boundaries and
acceptance tests. Historical evidence below is not automatically a new manual
review. The assessment distinguishes source-confirmed differences from missing executable
proof; the [verification record](../../openspec/changes/archive/2026-09-22-upgrade-pinned-reference-september-2026/verification.md)
reports the checks actually run.

| Surface / exact target source and tests | Current Go behavior, proof and disposition |
| --- | --- |
| Core lifecycle: `ai/src/generate-text/stream-text{,.test}.ts` | Cross-step text/reasoning IDs are corrected, including generator collisions; existing Anthropic replay failed first and now passes, with root and schema-parsed frontend assembly tests. Step overrides, stop conditions, usage, response history and cancellation have existing tests, not exhaustive interleaving proof. |
| Tool finish gating: `execute-tools-from-stream{,.test}.ts`, `is-tool-execution-allowed-finish-reason.ts` | Review promoted [#208](https://github.com/grafana/ai-sdk/issues/208) into this upgrade because recoverable malformed streams newly exposed executable calls after an error. Local dispatch now requires stop/tool-calls, approval observations remain, and continuation requires matching client results/denials. Root finish/approval matrix, real HTTP/core malformed-wrapper cases and schema-parsed frontend proof cover the correction; publication remains pending. |
| Retries/timeouts: `stream-text{,.test}.ts`, `stream-retry-attempt-boundary.ts` | Setup retries and semantic timeout tests exist; optional current-step stream retry is missing ([#209](https://github.com/grafana/ai-sdk/issues/209)). Abandoned FullStream cancellation remains a real bug ([#165](https://github.com/grafana/ai-sdk/issues/165)), not a scheduling adaptation. |
| Tool preparation/execution: `tool-order.ts`, `stream-language-model-call.ts`, `tool-caller-configuration.ts` and stream tests | Go sorts/filters/validates tools but lacks explicit ordering and repair/refinement ([#210](https://github.com/grafana/ai-sdk/issues/210)), caller-aware visibility/context and preliminary result execution APIs ([#211](https://github.com/grafana/ai-sdk/issues/211)). Manual provider allowedCallers is not core routing parity. |
| Tool ordering: `step-result.ts`, `to-response-messages.ts` and tests | Completion-order emission is implemented; mixed rejected/denied/executed result-list ordering remains [#166](https://github.com/grafana/ai-sdk/issues/166). Closed #161 is not recreated. Adjacent sibling tie normalization remains the documented Go scheduling deviation. |
| Agent: `ai/src/agent/tool-loop-agent{,.test}.ts`, settings and model-call events | Identity, 20-step default, option/context merge and helpers exist. Validated call preparation/templates/model-call hooks are missing ([#212](https://github.com/grafana/ai-sdk/issues/212)). Go Generate/Agent.Generate deliberately collect StreamText rather than dispatch provider DoGenerate; unary adapter tests do not prove that high-level path. |
| Output: `ai/src/generate-text/output{,.test}.ts`, `generate-text.ts` | Object/array/choice/JSON formats, validation and partial/element delivery have tests and snapshots. Generate/Agent.Generate omit terminal parsing for non-stop text ([#226](https://github.com/grafana/ai-sdk/issues/226)). OutputError/sentinel errors and channels are Go API adaptations, not identical TS error objects. |
| UI conversion/readers/validation: `ai/src/ui/{process-ui-message-stream,convert-to-model-messages,validate-ui-messages}.ts` and tests | Wire chunks pass exercised schema/replay cases; persisted tool structs lose title/toolMetadata/preliminary/raw-input distinctions. Incomplete-tool/data-part conversion and minimum Agent validation are narrower ([#213](https://github.com/grafana/ai-sdk/issues/213)). |
| Composed UI streams: `ai/src/ui-message-stream/create-ui-message-stream{,.test}.ts` | Framing/continuation, missing/panicking Execute and HTTP-failure drain are already fixed. Blocking Merge drains inside Execute; asynchronous registration is absent ([#181](https://github.com/grafana/ai-sdk/issues/181), per discussion and merged #183). New outcome/step-end hooks exceed its old acceptance: scope overlap is unresolved, not automatically duplicated. |
| React/HTTP: `react/src/use-{chat,completion,object}.ts` and UI tests, `ai/src/ui/chat.ts` | Actual hooks cover success, selected errors/stops, tool steps and approvals; headers, chunks and new ID/metadata scenarios pass. Regenerate/reconnect, callback/overlap and object failure/stop evidence remain [#214](https://github.com/grafana/ai-sdk/issues/214), beyond completed #36. |
| File URL preparation: `ai/src/prompt/convert-to-language-model-prompt{,.test}.ts`, provider-utils validated fetch | Core exposes SupportedURLs but does not download unsupported URLs. Native provider URL handling is not equivalent to automatic validated download ([#225](https://github.com/grafana/ai-sdk/issues/225)). |
| Provider V4: exact language-model call/prompt/content/stream/result/warning types | Synchronous interface source is unchanged across package refs; selected discriminators, finite ProviderWire witnesses and Go mapping inventory pass. Context/channels, raw JSON, flat unions and time.Time are adaptations. Optional presence losses are real; filename/format-name/denial-reason work belongs to [#109](https://github.com/grafana/ai-sdk/issues/109), [#114](https://github.com/grafana/ai-sdk/issues/114), [#108](https://github.com/grafana/ai-sdk/issues/108). Batch moved to a separate contract, not a new required LanguageModel method. |
| Anthropic/Vertex construction and controls: provider/options/language-model/prompt source and tests | Explicit Go credentials and no ambient Anthropic defaults remain intentional. Raw SDK construction is narrower than target URL normalization/validation; constructor API policy is tracked in [#228](https://github.com/grafana/ai-sdk/issues/228). Global and inline system/thinking/skill controls are missing ([#216](https://github.com/grafana/ai-sdk/issues/216)). No separately pinned google-vertex package or live Vertex attestation is claimed. |
| Anthropic models/tools: language-model and prepare-tools source/tests | Dated Vertex Claude-4 IDs can miss capability rules; March web-tool variants and request validation are incomplete ([#217](https://github.com/grafana/ai-sdk/issues/217)). Existing cache/multimodal/structured cases have request proof. Closed unmerged #204 is research, not implemented work. |
| Anthropic response/continuation: language-model, prompt, usage and stop mapping/tests | Caller metadata now survives server calls and web search/fetch outputs and replay. Focused unary/stream/error/continuation tests also exposed and corrected dropped web-search error results. Existing citations/signatures/fallback/usage proof remains; #29 finish mapping stays fixed. Retry eligibility differences remain explicit below. |
| SDK-backed transport: Anthropic/OpenAI language-model source/tests | Anthropic call headers are not applied; both adapters omit returned request/stream-response metadata and requested raw frames. Server-captured snapshots do not prove those result surfaces. Native adapter work [#229](https://github.com/grafana/ai-sdk/issues/229) is distinct from Gateway raw-output policy #116. |
| OpenAI requests: Responses converter/prepare-tools/tests | Assistant text now uses easy input; stored references and phase remain. Forced shell/local-shell/tool-search choices still differ ([#32](https://github.com/grafana/ai-sdk/issues/32)); allowedTools assumes functions ([#218](https://github.com/grafana/ai-sdk/issues/218)); schema normalization is incomplete ([#219](https://github.com/grafana/ai-sdk/issues/219)). |
| OpenAI controls/results: capabilities/options/converter/tests | Astra sampling matches target; configuration updates, async controls and effort validation are incomplete ([#220](https://github.com/grafana/ai-sdk/issues/220)). Generic multipart outputs, scalar cache breakpoints and custom uploaded references remain [#221](https://github.com/grafana/ai-sdk/issues/221). PR #164 reasoning-summary suppression is separate, not implicitly approved. |
| OpenAI streaming/continuation: Responses language-model, expand-parallel-tool-call and tests | Apply-patch finish reasons are corrected. Undeclared valid parallel wrappers expand into declared functions; declared/invalid wrappers retain normal behavior. Scalar stateful result groups reconstruct wrapper identity. Malformed JSON SSE emits errors and continues: exact imported first/last malformed frames remain untouched. Imported replay and focused tests pass; multipart grouping inherits #221. Retained Go incomplete-stream policy: plain EOF without a terminal/error event does not synthesize a provider finish, allowing core to distinguish premature closure; target unconditionally flushes a finish. Broader EOF policy alignment is a separate decision, not claimed by wrapper-input recovery. Existing logprobs #31 stays fixed. |
| Compatible Chat: chat language-model/options/converters/tests | Text/tools/output, malformed-event recovery and irregular indexes have evidence. Placeholder response metadata/zero timestamps are corrected and tested beyond UI snapshots. Builder-field override remains [#193](https://github.com/grafana/ai-sdk/issues/193), PR #194. Custom response metadata extractor hooks are not exposed by the generic Go adapter API. #28's documentation now exists; no duplicate package is created. |
| Bedrock Converse: language-model/options/model support/prompt/tools/tests | Existing requests, streams, usage and guardrail outcomes are covered selectively. Family/profile/structured/reasoning/document/tool routing differs ([#222](https://github.com/grafana/ai-sdk/issues/222)); selective guardContent is missing ([#223](https://github.com/grafana/ai-sdk/issues/223)). Its new unary intervention fixture is indexed but not imported before request support. Unsupported file-result URL warnings remain explicit Go behavior. |
| Mantle Responses: Mantle factory/tests and shared OpenAI adapter | Local producer corrections are not adopted by its published dependency. Owner approved producer-then-consumer delivery for the live-confirmed assistant-history defect ([#207](https://github.com/grafana/ai-sdk/issues/207)); automatic web-search includes are separately [#227](https://github.com/grafana/ai-sdk/issues/227). AWS routing/SigV4 are explicit adaptations; no production replacement or unmerged producer pin. |
| Gateway client: gateway-language-model, errors and tests | Byte/URL/header/presence witnesses match; unary warnings now survive. Grafana intentionally retains bounded strict response families and protected auth rather than permissive upstream parsing. Post-header transport retryability differs ([#224](https://github.com/grafana/ai-sdk/issues/224)). Async-job errors are excluded with unsupported async routes. |
| ProviderWire/Grafana service/fallback | Public-client contract/runtime tests are authoritative; there is no Vercel private-service oracle. Host auth/privacy/bounds/ordered fallback are explicit Grafana extensions. Main's #203 fixes default auto tool choice. Remaining response/request families reuse [#107](https://github.com/grafana/ai-sdk/issues/107), [#108](https://github.com/grafana/ai-sdk/issues/108), [#109](https://github.com/grafana/ai-sdk/issues/109), [#110](https://github.com/grafana/ai-sdk/issues/110), [#111](https://github.com/grafana/ai-sdk/issues/111), [#112](https://github.com/grafana/ai-sdk/issues/112), [#113](https://github.com/grafana/ai-sdk/issues/113), [#114](https://github.com/grafana/ai-sdk/issues/114), [#115](https://github.com/grafana/ai-sdk/issues/115), [#116](https://github.com/grafana/ai-sdk/issues/116). Provider configuration reuses [#117](https://github.com/grafana/ai-sdk/issues/117), [#118](https://github.com/grafana/ai-sdk/issues/118), [#119](https://github.com/grafana/ai-sdk/issues/119). |
| Middleware/registry: `ai/src/middleware/`, `ai/src/registry/` source/tests | Language-model wrappers, defaults/extraction and named/custom resolution have tests. Simulated streams lose content fields/metadata ([#215](https://github.com/grafana/ai-sdk/issues/215)). Grafana observability/enrichment/logging are extensions, not native TS telemetry/performance-result equivalents. |
| Harness | Inventory enforces exact bytes and all stream imports; no malformed wrapper input was repaired. Unary evaluation/guard-content/wrapper counterparts are indexed as unimported. Request queries/config breadth/shape enforcement remain [#30](https://github.com/grafana/ai-sdk/issues/30), [#33](https://github.com/grafana/ai-sdk/issues/33), [#34](https://github.com/grafana/ai-sdk/issues/34), [#35](https://github.com/grafana/ai-sdk/issues/35). All four fixture-provider commits coincide here, so #33 does not invalidate these byte checks; provider contract source is independently resolved. |

### Scope boundaries and unresolved decisions

This port implements language-model orchestration, Output, UI/SSE, selected
adapters and Grafana Gateway, not every upstream product family. Exact exports
identify these exclusions: embeddings, reranking/evaluation, image/video model
APIs, speech/transcription/translation, realtime/live, durable batch, upload and
managed-file/skill convenience APIs, RSC/other frameworks, and sandbox-provider
implementations. File parts, references and hosted image outputs inside supported
LanguageModel calls remain in scope. Legacy generateObject/streamObject repairText
APIs are excluded in favor of Output, not claimed implemented.

OpenAI Chat/legacy Completions, Bedrock Anthropic Invoke and Mantle default Chat
are absent adapters, not aliases for Responses/Converse. Open #48 has no body or
acceptance criteria: overlap with Mantle Chat needs a scope decision, not a new
likely duplicate. React realtime/MCP-app APIs do not define the supported hook
wire contract. Private Vercel service routing, billing and deployment are excluded.

Constructor URL policy (#228), UI outcome/step hooks versus #181, and the new APIs in
registered packages remain explicit unresolved design decisions. Registration
is not API approval or permanent deviation acceptance. Successful supported
default integrations remain required; the owner-approved publication deferral
for demonstrated Mantle assistant-history failure is specifically #207.

Every created/reused actionable package above has verified `upstream-sync`;
existing metadata was preserved. Closed matches were distinguished from live
owners. No comments, closures/reopenings or assignments were made.
...
| Cancellation and abort behavior | automated | Provider-independent `ui/stream-abort` fixtures compare actual `StreamText` cancellation UI chunks for no-output, partial-output, and pending-output streams against abort behavior originally re-attested from `ai@7.0.65` and replayed against the target; focused Go tests cover `GenerateText` cancellation during and immediately after tool execution. | Provider transport cancellation remains covered by provider-specific unit tests rather than timed replay fixtures. |
...
| Message and content part taxonomy | mixed | Provider request snapshots cover current message/content variants. The Anthropic `ui-tool-model-output` fixture verifies that persisted UI tool results apply `Tool.ToModelOutput` and reach the provider as multipart text/file content. | New content parts must include provider request assertions. Known Go conversion gaps versus `ai@7.0.107` include upstream `convertDataPart` support and some tool conversion semantics (`input-streaming` filtering default and output-denied default text); preserve existing Go behavior unless a focused parity change updates fixtures and docs. |
...
standalone Apache and isolated Gateway tests verify those dependencies. That work package did not change the registered upstream baseline; production
...
process against the same HTTP cases as `@ai-sdk/gateway@4.0.87`; real-command
