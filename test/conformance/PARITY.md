...
| Provider adapters | mixed | `recorded/` and `upstream/` fixtures exercise represented requests, stream events, usage and outputs; provider tests cover synthetic failures and local invariants, including OpenAI multipart function-result request conversion, SDK-submitted request/response metadata, and opt-in Anthropic/OpenAI raw event ordering. | No recorded/imported OpenAI request fixture exercises multipart result references or cache breakpoints; request snapshots alone do not prove live acceptance or returned SDK metadata. Focused Anthropic/Vertex and OpenAI returned request bytes match deterministic endpoints, but a caller-supplied transport that rewrites after the SDK capture boundary is outside the exact-wire guarantee. Anthropic's SDK filters malformed ping/unknown events without the parse/schema errors pinned upstream emits; raw frame values remain visible when opted in, but normalized errors are not synthesized (tracked by [#229](https://github.com/grafana/ai-sdk/issues/229)). Volatile SigV4 headers are excluded from snapshots. |
