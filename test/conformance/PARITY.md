...
| Provider adapters | mixed | `recorded/` and `upstream/` fixtures exercise represented requests, stream events, usage and outputs; provider tests cover synthetic failures and local invariants. | Synthetic HTTP tests are not provider recordings. Native Anthropic tests cover optional MCP token/enabled presence and context-derived unary timeout defaults, including explicit-timeout precedence. Request capture does not prove returned SDK metadata. Volatile SigV4 headers are excluded from snapshots. |
...
| Gateway runtime and Go client | mixed | Handler, differential and command tests exercise text, function/provider calls and results, framing, bounds, privacy, cancellation and ownership. Provider-only captures, deferred-result handler tests and authenticated native code-execution tests run through both clients. Separate MCP captures and both-client scenarios exercise narrow direct-Anthropic server options, configured-name metadata and deferred results; ordinary provider-only coverage remains. | Schema acceptance and runtime support differ. Permissive client parsing does not prove strict server output, privacy or resource bounds. |
...
- Provider-tool runtime tests cover history-aware correlation, preview finalization,
  presence-sensitive markers and bounded reviewed metadata without a
  Gateway executor or session. Unsupported compound outputs and pre-call image
  previews remain explicit failures. Fake native requests are transport evidence,
  not live provider acceptance or deployment proof.
- Hosted MCP options are limited to configured direct Anthropic routes. URL/token
  values reach only the selected native transport and caller-owned request, not
  normalized public output or metadata-only telemetry. Fallback and other backend
  types reject MCP before invocation even without tool definitions. HTTPS and
  configured-name checks are Grafana policy, not a private Vercel-service oracle;
  live remote egress and deployment activation remain unverified.
