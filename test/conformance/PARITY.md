...
| Grafana provider-wire transport | automated | Grafana conformance runs the real Grafana client through the public legacy server and downstream Anthropic replay. `providers/grafana/providerwire_server_test.go` runs shared canonical unary, ordered streaming, and post-error continuation scenarios against both the legacy client/handler and opt-in strict client/handler. Focused tests cover bounded unary, error, diagnostic, and complete-event reads. The `test/interop` harness uses distinct base URLs to run the pinned `@ai-sdk/gateway@4.0.33` client against both handlers, including rich unary replacement semantics, canonical tool-result file conversion, text/tools/files/sources, post-error continuation, redaction, and the strict public error/retry matrix. | This covers transparent transport behavior, not Grafana service behavior. Strict mode is intentionally opt-in during migration. The reusable Go runtime and server lifecycle have no direct upstream TypeScript server equivalent. HTTP 500 remains retryable to the pinned TypeScript client even when the strict envelope and Grafana classify the internal failure as non-retryable. |
| Provider-wire upstream encode compatibility | automated | Legacy provider-wire encoders retain upstream-compatible bytes while its decoders remain tolerant of the deployed Go dialect. The independent `gateway/providerwire/v4` package uses private bidirectional DTOs and handwritten pinned-baseline goldens for every supported request, generated-content, tool-output, file-data, and stream discriminator; strict decoders reject legacy unions and malformed variants. Raw HTTP and strict Grafana tests assert public warning/ID/timestamp preservation plus omission of backend request bodies, response headers/bodies, and `modelId`. Inline bytes/base64 and URL variants remain covered by provider tests and the dual-handler `test/interop` harness. | Strict V4 is the intended canonical endpoint, but legacy decoding and provider custom JSON ownership remain available until a coordinated breaking migration. |
