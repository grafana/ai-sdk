...
| ID | Valid pinned distinction | Evidence scenario and path | Original Go semantic loss | Phase 1 executable witness | Required Phase 2 change | Phase 2 target |
| --- | --- | --- | --- | --- | --- | --- |
| `numeric-settings` | Fractional `maxOutputTokens`, `topK`, and `seed` numbers | `presence-losses` `/requests/0/body/maxOutputTokens`, `/topK`, and `/seed` | Go narrowed each upstream `number` to `*int`; no `CallOptions` value could carry a fraction. | `TestProviderWireV4Loss_FractionalNumericSettings` | Represent the exact numeric domain or intentionally narrow it in a separately approved contract change. | `TestProviderWireV4Contract_NumericSettings` (resolved) |
| `include-raw-chunks-presence` | Explicit `includeRawChunks: false` versus absence | `unary-settings` `/requests/0/body/includeRawChunks` | The non-pointer boolean had one zero value for both semantic states. | `TestProviderWireV4Loss_ExplicitFalseIncludeRawChunks` | Use a presence-aware boolean. | `TestProviderWireV4Contract_IncludeRawChunksPresence` (resolved) |
| `optional-string-presence` | Explicit empty optional strings versus absence for response-format name/description, function description, prompt and tool-result filenames, approval reason, and execution-denied reason | `presence-losses` and `streaming-prompt-tools` at the corresponding nested paths | Plain string fields had one zero value for both semantic states. | `TestProviderWireV4Loss_ExplicitEmptyOptionalStrings` | Use pointers or another presence-preserving representation for optional strings. | `TestProviderWireV4Contract_OptionalStringPresence` (resolved) |
| `tool-call-provider-executed-presence` | Explicit `providerExecuted: false` versus absence on a tool call | `presence-losses` `/requests/0/body/prompt/2/content/1/providerExecuted` | The non-pointer boolean had one zero value for both semantic states. | `TestProviderWireV4Loss_ExplicitFalseToolCallProviderExecuted` | Use a presence-aware boolean on the tool-call request arm. | `TestProviderWireV4Contract_ToolCallProviderExecutedPresence` (resolved) |
| `empty-inline-text-file-data` | Required inline-text file data `{type:"text",text:""}` | `presence-losses` `/requests/0/body/prompt/1/content/1/data` | Public typed fields and constructors could not select the empty text variant: `DataContent{Text:""}` failed validation, while public JSON decoding created a valid value with the same exported fields by setting private variant state. Provider JSON decoding was not a transport-neutral construction contract for an external V4 mapper. | `TestProviderWireV4Loss_RequiredEmptyInlineTextFileData` | Expose a presence-preserving tagged file-data representation or constructor that an external V4 mapper can inspect without treating provider JSON output as authority. | `TestProviderWireV4Contract_EmptyInlineTextFileData` (resolved) |
